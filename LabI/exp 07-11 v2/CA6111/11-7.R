...
CorregirCampoMes_frollmean <- function(pcampo) {
  tbl <- dataset[, .(v1 = shift(get(pcampo), 2, type = "lag"),
                     v2 = shift(get(pcampo), 1, type = "lag"),
                     v3 = shift(get(pcampo), 1, type = "lead"),
                     v4 = shift(get(pcampo), 2, type = "lead")),
                 by = numero_de_cliente]
  
  tbl[, promedio := rowMeans(.SD, na.rm = TRUE), .SDcols = c("v1", "v2", "v3", "v4")]
...
CorregirVariables_frollmean <- function(dataset){
...
switch(PARAM$metodo,
       ...
       "frollmean"            = CorregirVariables_frollmean(dataset)
)
